...
TEST(ProcessLineTest, ValidTwoNumbers) {
    int h1 = 0, h2 = 0;
    EXPECT_TRUE(processLine("100 200", h1, h2));
    EXPECT_EQ(h1, 200);
    EXPECT_EQ(h2, 0);
}
...
TEST(ProcessLineTest, ValidThreeNumbers) {
    int h1 = 0, h2 = 0;
    EXPECT_TRUE(processLine("10 20 30", h1, h2));
    EXPECT_EQ(h1, 20);
    EXPECT_EQ(h2, 30);
}
...
TEST(ProcessLineTest, InvalidWithLetters) {
    ...
    EXPECT_FALSE(processLine("abc 123 456", h1, h2));
...
TEST(ProcessLineTest, InvalidWithSpecialCharacters) {
    int h1 = 0, h2 = 0;
    testing::internal::CaptureStdout();
    EXPECT_FALSE(processLine("123 !@# 456", h1, h2));
    std::string output = testing::internal::GetCapturedStdout();
    EXPECT_EQ(output, "false\n");
}
/**
